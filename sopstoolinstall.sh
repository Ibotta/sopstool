...
usage() {
  this=$1
  cat <<EOF
$this: download binaries for Ibotta/sopstool

Usage: $this [-b bindir] [-o OS] [-a ARCH] [-d] [tag]
  ...
  -d turns on debug logging
   [tag] is a tag from https://github.com/Ibotta/sopstool/releases
...
parse_args() {
  BINDIR="${BINDIR:-./bin}"
  while getopts "b:o:a:dxh?" arg; do
    case "${arg}" in
      b) BINDIR="${OPTARG}" ;;
      o) TARGET_OS="${OPTARG}" ;;
